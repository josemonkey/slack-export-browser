...
// RichTextBlocks may contain RichTextElements, which may in turn contain children that are RichTextElements.
...
export default function RichTextElement({ element, allChannels, allUsers }) {
  ...
  if (element.type === 'rich_text_section') {
    ...
    if (element.unicode && !isNaN(element.unicode)) {
      return <span>{String.fromCodePoint(parseInt(element.unicode, 16))}</span>
    } else {
      return <></>
    }
  } else if (element.type === 'link') {
    var linkText = element.text ? element.text : element.url
    return <a href={element.url}>{linkText}</a>
    ...
    if (otherChannel && otherChannel.name)
      return (
        <Link href={`/channels/${otherChannel.name}`}>
          #{otherChannel.name}
        </Link>
      )
    else {
      return (
        <span className={utilStyles.error}>
          [Invalid channel reference, id={element.channel_id} ]
        </span>
      )
    }
    ...
    return <User userData={allUsers} userId={element.user_id} />
  } else {
    return (
      <div className={utilStyles.error}>Unknown RT Element: {element.type}</div>
    )
  }
}
